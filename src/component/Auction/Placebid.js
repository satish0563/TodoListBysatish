...
export default function Placebid() {
  ...
  if (posts.auctionlistval.length !== 0) {
    posts = posts.auctionlistval.data.auctionList.filter(
      ...
      <Container>
        <Row>
          {usersInfo.map((postItem, item) => (
            <div
              key={item}
              className="card custom-card col-lg-3 d-flex align-items-stretch mt-4 "
              xs={6}
              md={3}
              style={{ width: "1120rem" }}
            >
              ...
              <Modal
                ...
                closeTimeoutMS={500}
              >
                <div className="aligncentre" onClick={toggleModal}>
                  Increment Rules
                </div>
                <div className="buttonmodel" onClick={toggleModal}>
                  <CloseButton />{" "}
                </div>
                <hr></hr>
                <p>
                  Here's the Minimum amount your bid needs to be raised given
                  the currnt bid on the item
                </p>
              ...
              <Modal
                ...
                closeTimeoutMS={500}
              >
                <div className="aligncentre" onClick={toggleModalBid}>
                  Bid Placed
                </div>
                <div className="buttonmodel" onClick={toggleModalBid}>
                  <CloseButton />{" "}
                </div>
                ...
                {postItem.bidFromOthers.map((itemval, index) => (
                  <p className="alignleftmodle">
                    ...
                    <span className="alignrightmodle">
                      {" "}
                      ${itemval.bids[0].bidPrice}
                    </span>
                    <hr></hr>
                  </p>
                ))}
              ...
              <div className="aligncentre">
                {" "}
                Current Bid: {postItem.currencySymbol}
                {postItem.currentBidAmount}
              </div>
              <br></br>
              ...
              >
                <Form.Control
                  ...
                  placeholder="Enter The Bid Ammount"
                />
              </Form.Group>

              <Row>
                <div onClick={toggleModal} className="textplacebid">
                  {" "}
                  View Bid Increment
                </div>
                <br></br>
              ...
              {state.checkedA ? (
                ...
                >
                  <Form.Control
                    type="text"
                    placeholder="Enter The Max Bid Ammount"
                  />
                </Form.Group>
              ) : (
                ""
  ...
  return posts.loading ? showLoader() : renderData(posts);
}
